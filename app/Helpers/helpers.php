...
if (!function_exists('get_option')) {
    ...
    function get_option($key, bool $decode = false, $locale = null, $associative = false): mixed
    {
        ...
        return $decode ? optional(optional(json_decode($option, $associative))->value) ?? null : $option->value ?? null;
    }
}
...
if (!function_exists('get_tax')) {
    function get_tax($key, $amount)
    {
        $tax = get_option($key);
        if ($tax['type'] == 'percentage') {
            $tax_amount = ($amount / 100) * $tax['rate'];
        }
        return $tax_amount ?? $tax['rate'];
    }
}
...
if (!function_exists('secondsToTime')){
    function secondsToTime($seconds, $format = null) {
        return Carbon::parse($seconds)->format('H:i:s');
    }
}
