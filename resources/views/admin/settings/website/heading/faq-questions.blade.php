        ...
        <div class="tab-content tab-bordered" id="myTab3Content">
            ...
            @foreach($languages->value as $key => $value)
                <div class="tab-pane fade {{ $i == 0 ? 'active' : null }} show" id="{{ $key }}-latest-news" role="tabpanel" aria-labelledby="{{ $key }}-latest-news-tab">
                    <form class="ajaxform" action="{{ route('admin.settings.website.heading.update-faq-questions') }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="short_title" id="short_title" class="form-control" value="{{ $headings['heading.faq-questions'][$key]['short_title'] ?? null }}" required>
                        </div>

                        <div class="form-group">
                            <label for="title" class="required">{{ __('Title') }} ({{ $key }})</label>
                            <input type="text" name="title" id="title" class="form-control" value="{{ $headings['heading.faq-questions'][$key]['title'] ?? null }}" required>
                        </div>

                        <div class="form-group">
                            <label for="description" class="required">{{ __('Description') }} ({{ $key }})</label>
                            <textarea name="description" id="description" class="form-control" rows="5" required>{{ $headings['heading.faq-questions'][$key]['description'] ?? null }}</textarea>
                        </div>
